...
Options.PlaySound = false -- Play a sound when a new queue appears
...
local BlankTex = "Interface\\AddOns\\Track_CallToArms\\Blank.tga"
local BarTex = "Interface\\AddOns\\Track_CallToArms\\Ferous.tga"
local Font = "Interface\\Addons\\Track_CallToArms\\PTSans.ttf"

local StartMessage = "|cffe50c11Track|r |cffeaeaeaCall To Arms: %s %s bonus is active!|r"
local EndedMessage = "|cffe50c11Track|r |cffeaeaeaCall To Arms: %s %s bonus has ended.|r"
local InvalidRole = "|cffe50c11Track|r |cffeaeaeaCall To Arms: Your class cannot perform the role of %s.|r"
...
CallToArms.ToggleButton.Label:SetFont("Interface\\Addons\\Track_CallToArms\\Arial.ttf", 10)
...
function CallToArms:PLAYER_ENTERING_WORLD()
	if (not self.FirstRun) then
		...
		if (UnitLevel("player") ~= 110) then -- We're only interested in max level information here.
...
function CallToArms:VARIABLES_LOADED()
	if (not Track_CallToArms) then
		Track_CallToArms = {}
	end

	for k, v in pairs(Track_CallToArms) do
		Options[k] = v
	end
	...
	for i = 1, self.NumHeaders do
		if (not Track_CallToArms["Enable"..self.HeadersByIndex[i].DungeonName]) then
...
function CallToArms:Minimize()
	...
	Track_CallToArms.Minimized = true
	
	self.ToggleButton.Label:SetPoint("CENTER", self.ToggleButton, 0.5, 0)
	self.ToggleButton.Label:SetFont("Interface\\Addons\\Track_CallToArms\\Arial.ttf", 14)
...
function CallToArms:Maximize()
	...
	Options.Minimized = false
	Track_CallToArms.Minimized = false
	
	self.ToggleButton.Label:SetPoint("CENTER", self.ToggleButton, 0.5, -4)
	self.ToggleButton.Label:SetFont("Interface\\Addons\\Track_CallToArms\\Arial.ttf", 10)
	self.ToggleButton.Label:SetText("▬")
...
function CallToArms:NewModule(name, id, subtypeid)
	...
	for i = 1, 3 do
		...
		Bar.FlashAnim.In1:SetSmoothing("inout")
		...
		Bar.FlashAnim.Out1:SetSmoothing("inout")
		...
		Bar.FlashAnim.In2:SetSmoothing("inout")
		...
		Bar.FlashAnim.Out2:SetSmoothing("inout")
		...
		Bar.FlashAnim.In3:SetSmoothing("inout")
		...
		Bar.FlashAnim.Out3:SetSmoothing("inout")
		...
		Bar.FlashAnim.Width:SetSmoothing("out")
		...
		Bar.FlashAnim.Height:SetSmoothing("out")
...
local CreateTrackCTAConfig = function()
	local Config = CreateFrame("Frame", "TrackCallToArmsConfig", UIParent)
	...
	Config.Text:SetText("|cffe50c11Track|r " .. BATTLEGROUND_HOLIDAY .. " " .. GetAddOnMetadata("Track_CallToArms", "Version"))
	...
	UpdateIntOption.EditBox.Hook = function(value)
		...
		Track_CallToArms.UpdateInterval = value
	end
	...
	WindowAlphaOption.EditBox.Hook = function(value)
		...
		Track_CallToArms.WindowAlpha = value
	...
	ConfigWindow.Boxes[1]:SetScript("OnMouseUp", function(self)
		if Options.AnnounceStart then
			Track_CallToArms.AnnounceStart = false
			Options.AnnounceStart = false
			
			self.Tex:SetVertexColor(0.8, 0, 0)
		else
			Track_CallToArms.AnnounceStart = true
	...
	ConfigWindow.Boxes[2]:SetScript("OnMouseUp", function(self)
		if Options.AnnounceEndings then
			Track_CallToArms.AnnounceEndings = false
			Options.AnnounceEndings = false
			
			self.Tex:SetVertexColor(0.8, 0, 0)
		else
			Track_CallToArms.AnnounceEndings = true
	...
	ConfigWindow.Boxes[3]:SetScript("OnMouseUp", function(self)
		if Options.FilterRole then
			Track_CallToArms.FilterRole = false
			...
			Track_CallToArms.FilterRole = true
	...
	ConfigWindow.Boxes[4]:SetScript("OnMouseUp", function(self)
		if Options.MinimizeInGroup then
			Track_CallToArms.MinimizeInGroup = false
			...
			Track_CallToArms.MinimizeInGroup = true
	...
	for i = 1, CallToArms.NumHeaders do
		...
		if (Track_CallToArms["Enable"..DungeonName] == nil) then
			Track_CallToArms["Enable"..DungeonName] = true
		end
		
		if Track_CallToArms["Enable"..DungeonName] then
		...
		Checkbox:SetScript("OnMouseUp", function(self)
			if Track_CallToArms["Enable"..self.DungeonName] then
				Track_CallToArms["Enable"..self.DungeonName] = false
				
				CallToArms.HeadersByIndex[self.Index]:Disable()
				
				self.Tex:SetVertexColor(0.8, 0, 0)
			else
				Track_CallToArms["Enable"..self.DungeonName] = true
...
local SlashCommand = function(cmd)
	...
	elseif (cmd == "config") then
		if (not TrackCallToArmsConfig) then
			CreateTrackCTAConfig()
			
			return
		end
		
		if TrackCallToArmsConfig:IsShown() then
			TrackCallToArmsConfig:Hide()
		else
			TrackCallToArmsConfig:Show()
		end
	else
		if CallToArms:IsShown() then
			CallToArms:Hide()
...
SLASH_TRACKCALLTOARMS1 = "/tcta"
SlashCmdList["TRACKCALLTOARMS"] = SlashCommand